...
Emoji.searchEmojiByName = function searchEmojiByName(name) {
  name = name.toLowerCase();
  return (
    emojis.filter((item) => item.name.toLowerCase().indexOf(name) > -1) || []
  );
};
...
Emoji.decodeEmoji = function decodeEmoji(str) {
  ...
  return str
    ...
    .map(function (value, index) {
      ...
      if (r) {
        return Emoji.findEmojiByKey(r.key).unicode;
      } else {
        return value;
      }
    })
 ...
 * @return {array}
 */
Emoji.getEmojiByCategoryAndSkinTone = function getEmojiByCategoryAndSkinTone(
  tone,
  category
) {
  tone = tone.toLowerCase();
  ...
  if (tone == "no skin tone") {
    return emojis.filter(
      (item) =>
        item.name.toLowerCase().indexOf("tone") < 0 &&
        item.group.toLowerCase() == category
    );
  } else {
    return emojis.filter(
      (item) =>
        item.name.toLowerCase().indexOf(tone) > -1 &&
        item.group.toLowerCase() == category
    );
  }
};
